            ...
            let liquidities: TPool[] = Liquidities.reduce(
                (result: TPool[], liquidity, index: number) => {
                    ...
                    if (liquidityInfo) {
                        ...
                        const stakingAddressFromBackend =
                            liquidityInfo.stakingAddress;
                        ...
                        // get aprs for the staking contract existing in frontend.
                        // ignore all if the staking contract is not existing in frontend.
                        let groupedApr: { [key: string]: string } = {},
                            groupedConfig: { [key: string]: TPoolConfig } = {};
                        if (typeof stakingAddressFromBackend === "string") {
                            groupedApr = {
                                [stakingAddressFromBackend]:
                                    liquidityInfo.apr as string,
                            };
                            groupedConfig = {
                                [stakingAddressFromBackend]:
                                    liquidityInfo.config as TPoolConfig,
                            ...
                            const apr = liquidityInfo.apr as string[];
                            const config =
                                liquidityInfo.config as TPoolConfig[];
                            stakingAddressFromBackend?.forEach(
                                (address, index) => {
                                    groupedApr[address] = apr[index];
                                    groupedConfig[address] = config[index];
                                }
                            );
                        }
                        ...
                        const apr =
                            typeof stakingAddress === "string"
                                ? groupedApr[stakingAddress]
                                : stakingAddress?.map(
                                      (address) => groupedApr[address]
                                  );
                        const config =
                            typeof stakingAddress === "string"
                                ? groupedConfig[stakingAddress]
                                : stakingAddress?.map(
                                      (address) => groupedConfig[address]
                                  );
